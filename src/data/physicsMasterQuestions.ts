import { Question } from '../types';
import { UNITS_BOOK_QUESTIONS } from './unitsQuestions';
import { VECTORS_BOOK_QUESTIONS } from './vectorsQuestions';
import { MOTION_BOOK_QUESTIONS } from './motionQuestions';
import { NARAYANA_MOTION_IN_STRAIGHT_LINE_QUESTIONS } from './narayanaMotionInStraightLineQuestions';
import { PHYSICS_SMART_BOOKLET_MOTION_QUESTIONS } from './physicsSmartBookletMotionInStraightLineQuestions';
import { YCT_MOTION_IN_PLANE_QUESTIONS } from './yctMotionInPlaneQuestions';
import { NARAYANA_MOTION_IN_PLANE_QUESTIONS } from './narayanaMotionInPlaneQuestions';
import {
  MOTION_2D_QUESTIONS,
  LAWS_OF_MOTION_QUESTIONS,
  NEWTONS_LAWS_OF_MOTION_QUESTIONS,
  FRICTION_QUESTIONS,
  WORK_ENERGY_POWER_QUESTIONS,
  ROTATIONAL_MOTION_QUESTIONS
} from './physicsChaptersPart1';
import { GRAVITATION_BOOK_QUESTIONS } from './gravitationQuestions';
import {
  ELASTICITY_BOOK_QUESTIONS,
  ELASTICITY_QUESTIONS,
  SURFACE_TENSION_BOOK_QUESTIONS,
  SURFACE_TENSION_QUESTIONS,
  FLUID_MECHANICS_BOOK_QUESTIONS,
  FLUID_MECHANICS_QUESTIONS,
  THERMAL_PROPERTIES_BOOK_QUESTIONS,
  THERMAL_PROPERTIES_QUESTIONS,
  THERMODYNAMICS_PHYSICS_BOOK_QUESTIONS,
  THERMODYNAMICS_PHYSICS_QUESTIONS,
  TRANSMISSION_OF_HEAT_BOOK_QUESTIONS,
  TRANSMISSION_OF_HEAT_QUESTIONS,
  KINETIC_THEORY_QUESTIONS
} from './physicsChaptersPart2';
import {
  SHM_BOOK_QUESTIONS,
  SHM_QUESTIONS,
  OSCILLATIONS_SHM_QUESTIONS,
  WAVES_BOOK_QUESTIONS,
  WAVES_QUESTIONS,
  WAVES_ACOUSTICS_QUESTIONS,
  CAPACITANCE_QUESTIONS,
  CURRENT_ELECTRICITY_QUESTIONS,
  MAGNETISM_MATTER_QUESTIONS
} from './physicsChaptersPart3';
import { ELECTROSTATICS_BOOK_QUESTIONS } from './electrostaticsQuestions';
import { MAGNETISM_BOOK_QUESTIONS } from './magnetismQuestions';
import {
  EMI_QUESTIONS,
  ALTERNATING_CURRENT_QUESTIONS,
  EM_WAVES_QUESTIONS,
  RAY_OPTICS_QUESTIONS,
  WAVE_OPTICS_QUESTIONS,
  DUAL_NATURE_QUESTIONS,
  ATOMS_PHYSICS_QUESTIONS,
  NUCLEI_PHYSICS_QUESTIONS,
  SEMICONDUCTORS_QUESTIONS
} from './physicsChaptersPart4';
import { DUAL_NATURE_XRAYS_QUESTIONS } from './dualNatureXRaysQuestions';
import { ATOMIC_NUCLEAR_PHYSICS_QUESTIONS } from './atomicNuclearPhysicsQuestions';
import { SELF_EVALUATION_TEST_26_QUESTIONS } from './selfEvaluationTest26Questions';
import { SELF_EVALUATION_TEST_27_QUESTIONS } from './selfEvaluationTest27Questions';
import { SEMICONDUCTOR_AIIMS_AR_QUESTIONS } from './semiconductorAIIMSARQuestions';
import { ELECTRONICS_CIRCUIT_AND_GRAPHICAL_QUESTIONS } from './electronicsGraphicalAndCircuitQuestions';
import { SELF_EVALUATION_TEST_28_QUESTIONS } from './selfEvaluationTest28Questions';
import { ALL_COMMUNICATION_ADVANCED_QUESTIONS } from './communicationAdvancedQuestions';
import {
  UNIVERSAL_PLANE_MIRROR_QUESTIONS,
  UNIVERSAL_SPHERICAL_MIRROR_QUESTIONS,
  ALL_UNIVERSAL_RAY_OPTICS_QUESTIONS
} from './universalRayOpticsQuestions';
import { UNIVERSAL_REFRACTION_PLANE_QUESTIONS } from './universalRefractionPlaneQuestions';
import { UNIVERSAL_TIR_QUESTIONS } from './universalTIRQuestions';

import {
  ALL_NCERT_PHYSICS_QUESTIONS,
  NCERT_PHYSICAL_WORLD_QUESTIONS,
  NCERT_UNITS_QUESTIONS,
  NCERT_MOTION_STRAIGHT_LINE_QUESTIONS,
  NCERT_MOTION_PLANE_QUESTIONS,
  NCERT_LAWS_OF_MOTION_QUESTIONS,
  NCERT_WORK_ENERGY_POWER_QUESTIONS,
  NCERT_ROTATIONAL_MOTION_QUESTIONS,
  NCERT_GRAVITATION_QUESTIONS,
  NCERT_SOLIDS_QUESTIONS,
  NCERT_FLUIDS_QUESTIONS,
  NCERT_THERMAL_PROPERTIES_QUESTIONS,
  NCERT_THERMODYNAMICS_QUESTIONS,
  NCERT_KINETIC_THEORY_QUESTIONS,
  NCERT_OSCILLATIONS_QUESTIONS,
  NCERT_WAVES_QUESTIONS,
  NCERT_ELECTRIC_CHARGES_QUESTIONS,
  NCERT_ELECTROSTATIC_POTENTIAL_QUESTIONS,
  NCERT_CURRENT_ELECTRICITY_QUESTIONS,
  NCERT_MOVING_CHARGES_MAGNETISM_QUESTIONS,
  NCERT_MAGNETISM_MATTER_QUESTIONS,
  NCERT_EMI_QUESTIONS,
  NCERT_ALTERNATING_CURRENT_QUESTIONS,
  NCERT_RAY_OPTICS_QUESTIONS,
  NCERT_WAVE_OPTICS_QUESTIONS,
  NCERT_DUAL_NATURE_QUESTIONS,
  NCERT_EM_WAVES_QUESTIONS,
  NCERT_ATOMS_QUESTIONS,
  NCERT_NUCLEI_QUESTIONS,
  NCERT_SEMICONDUCTORS_QUESTIONS,
  NCERT_COMMUNICATION_QUESTIONS
} from './ncertPhysicsQuestions';

import {
  ALLEN_UNITS_EXERCISE_1_QUESTIONS,
  ALLEN_UNITS_EXERCISE_2_QUESTIONS,
  ALLEN_UNITS_EXERCISE_3_QUESTIONS,
  ALL_ALLEN_UNITS_DIMENSIONS_QUESTIONS
} from './allenUnitsDimensionsQuestions';
import {
  ALL_ALLEN_KINEMATICS_QUESTIONS,
  ALLEN_KINEMATICS_EXERCISE_1_QUESTIONS,
  ALLEN_KINEMATICS_EXERCISE_2_QUESTIONS,
  ALLEN_KINEMATICS_EXERCISE_3_QUESTIONS
} from './allenKinematicsQuestions';
import {
  ALL_PHYSICS_SMART_BOOKLET_VECTORS_QUESTIONS,
  PHYSICS_SMART_BOOKLET_VECTORS_ILLUSTRATIONS,
  PHYSICS_SMART_BOOKLET_VECTORS_TOPIC_QUESTIONS,
  PHYSICS_SMART_BOOKLET_VECTORS_PYQ_QUESTIONS
} from './physicsSmartBookletVectorsQuestions';
import {
  ALL_LAWS_OF_MOTION_NTA_QUESTIONS,
  LAWS_OF_MOTION_PART1_QUESTIONS,
  LAWS_OF_MOTION_PART2_QUESTIONS,
  LAWS_OF_MOTION_PART3_QUESTIONS,
  LAWS_OF_MOTION_PART4_QUESTIONS
} from './lawsOfMotionNTAQuestions';

export {
  ALLEN_UNITS_EXERCISE_1_QUESTIONS,
  ALLEN_UNITS_EXERCISE_2_QUESTIONS,
  ALLEN_UNITS_EXERCISE_3_QUESTIONS,
  ALL_ALLEN_UNITS_DIMENSIONS_QUESTIONS,
  ALL_ALLEN_KINEMATICS_QUESTIONS,
  ALLEN_KINEMATICS_EXERCISE_1_QUESTIONS,
  ALLEN_KINEMATICS_EXERCISE_2_QUESTIONS,
  ALLEN_KINEMATICS_EXERCISE_3_QUESTIONS,
  ALL_PHYSICS_SMART_BOOKLET_VECTORS_QUESTIONS,
  PHYSICS_SMART_BOOKLET_VECTORS_ILLUSTRATIONS,
  PHYSICS_SMART_BOOKLET_VECTORS_TOPIC_QUESTIONS,
  PHYSICS_SMART_BOOKLET_VECTORS_PYQ_QUESTIONS,
  UNITS_BOOK_QUESTIONS,
  VECTORS_BOOK_QUESTIONS,
  MOTION_BOOK_QUESTIONS,
  MOTION_2D_QUESTIONS,
  LAWS_OF_MOTION_QUESTIONS,
  NEWTONS_LAWS_OF_MOTION_QUESTIONS,
  ALL_LAWS_OF_MOTION_NTA_QUESTIONS,
  LAWS_OF_MOTION_PART1_QUESTIONS,
  LAWS_OF_MOTION_PART2_QUESTIONS,
  LAWS_OF_MOTION_PART3_QUESTIONS,
  LAWS_OF_MOTION_PART4_QUESTIONS,
  FRICTION_QUESTIONS,
  WORK_ENERGY_POWER_QUESTIONS,
  ROTATIONAL_MOTION_QUESTIONS,
  GRAVITATION_BOOK_QUESTIONS,
  ELASTICITY_BOOK_QUESTIONS,
  ELASTICITY_QUESTIONS,
  SURFACE_TENSION_BOOK_QUESTIONS,
  SURFACE_TENSION_QUESTIONS,
  FLUID_MECHANICS_BOOK_QUESTIONS,
  FLUID_MECHANICS_QUESTIONS,
  THERMAL_PROPERTIES_BOOK_QUESTIONS,
  THERMAL_PROPERTIES_QUESTIONS,
  THERMODYNAMICS_PHYSICS_BOOK_QUESTIONS,
  THERMODYNAMICS_PHYSICS_QUESTIONS,
  TRANSMISSION_OF_HEAT_BOOK_QUESTIONS,
  TRANSMISSION_OF_HEAT_QUESTIONS,
  KINETIC_THEORY_QUESTIONS,
  SHM_BOOK_QUESTIONS,
  SHM_QUESTIONS,
  OSCILLATIONS_SHM_QUESTIONS,
  WAVES_BOOK_QUESTIONS,
  WAVES_QUESTIONS,
  WAVES_ACOUSTICS_QUESTIONS,
  ELECTROSTATICS_BOOK_QUESTIONS,
  CAPACITANCE_QUESTIONS,
  CURRENT_ELECTRICITY_QUESTIONS,
  MAGNETISM_BOOK_QUESTIONS,
  MAGNETISM_MATTER_QUESTIONS,
  EMI_QUESTIONS,
  ALTERNATING_CURRENT_QUESTIONS,
  EM_WAVES_QUESTIONS,
  RAY_OPTICS_QUESTIONS,
  WAVE_OPTICS_QUESTIONS,
  DUAL_NATURE_QUESTIONS,
  ATOMS_PHYSICS_QUESTIONS,
  NUCLEI_PHYSICS_QUESTIONS,
  SEMICONDUCTORS_QUESTIONS,
  ALL_NCERT_PHYSICS_QUESTIONS,
  NCERT_PHYSICAL_WORLD_QUESTIONS,
  NCERT_UNITS_QUESTIONS,
  NCERT_MOTION_STRAIGHT_LINE_QUESTIONS,
  NCERT_MOTION_PLANE_QUESTIONS,
  NCERT_LAWS_OF_MOTION_QUESTIONS,
  NCERT_WORK_ENERGY_POWER_QUESTIONS,
  NCERT_ROTATIONAL_MOTION_QUESTIONS,
  NCERT_GRAVITATION_QUESTIONS,
  NCERT_SOLIDS_QUESTIONS,
  NCERT_FLUIDS_QUESTIONS,
  NCERT_THERMAL_PROPERTIES_QUESTIONS,
  NCERT_THERMODYNAMICS_QUESTIONS,
  NCERT_KINETIC_THEORY_QUESTIONS,
  NCERT_OSCILLATIONS_QUESTIONS,
  NCERT_WAVES_QUESTIONS,
  NCERT_ELECTRIC_CHARGES_QUESTIONS,
  NCERT_ELECTROSTATIC_POTENTIAL_QUESTIONS,
  NCERT_CURRENT_ELECTRICITY_QUESTIONS,
  NCERT_MOVING_CHARGES_MAGNETISM_QUESTIONS,
  NCERT_MAGNETISM_MATTER_QUESTIONS,
  NCERT_EMI_QUESTIONS,
  NCERT_ALTERNATING_CURRENT_QUESTIONS,
  NCERT_RAY_OPTICS_QUESTIONS,
  NCERT_WAVE_OPTICS_QUESTIONS,
  NCERT_DUAL_NATURE_QUESTIONS,
  NCERT_EM_WAVES_QUESTIONS,
  NCERT_ATOMS_QUESTIONS,
  NCERT_NUCLEI_QUESTIONS,
  NCERT_SEMICONDUCTORS_QUESTIONS,
  NCERT_COMMUNICATION_QUESTIONS,
  DUAL_NATURE_XRAYS_QUESTIONS,
  ATOMIC_NUCLEAR_PHYSICS_QUESTIONS,
  SELF_EVALUATION_TEST_26_QUESTIONS,
  SELF_EVALUATION_TEST_27_QUESTIONS,
  SEMICONDUCTOR_AIIMS_AR_QUESTIONS,
  ELECTRONICS_CIRCUIT_AND_GRAPHICAL_QUESTIONS,
  SELF_EVALUATION_TEST_28_QUESTIONS,
  ALL_COMMUNICATION_ADVANCED_QUESTIONS,
  UNIVERSAL_PLANE_MIRROR_QUESTIONS,
  UNIVERSAL_SPHERICAL_MIRROR_QUESTIONS,
  ALL_UNIVERSAL_RAY_OPTICS_QUESTIONS,
  UNIVERSAL_REFRACTION_PLANE_QUESTIONS,
  UNIVERSAL_TIR_QUESTIONS,
  NARAYANA_MOTION_IN_STRAIGHT_LINE_QUESTIONS,
  PHYSICS_SMART_BOOKLET_MOTION_QUESTIONS,
  YCT_MOTION_IN_PLANE_QUESTIONS,
  NARAYANA_MOTION_IN_PLANE_QUESTIONS
};

export const ALL_PHYSICS_MASTER_QUESTIONS: Question[] = [
  ...ALL_NCERT_PHYSICS_QUESTIONS,
  ...UNITS_BOOK_QUESTIONS,
  ...ALL_ALLEN_UNITS_DIMENSIONS_QUESTIONS,
  ...ALL_ALLEN_KINEMATICS_QUESTIONS,
  ...VECTORS_BOOK_QUESTIONS,
  ...ALL_PHYSICS_SMART_BOOKLET_VECTORS_QUESTIONS,
  ...MOTION_BOOK_QUESTIONS,
  ...NARAYANA_MOTION_IN_STRAIGHT_LINE_QUESTIONS,
  ...PHYSICS_SMART_BOOKLET_MOTION_QUESTIONS,
  ...YCT_MOTION_IN_PLANE_QUESTIONS,
  ...NARAYANA_MOTION_IN_PLANE_QUESTIONS,
  ...MOTION_2D_QUESTIONS,
  ...LAWS_OF_MOTION_QUESTIONS,
  ...ALL_LAWS_OF_MOTION_NTA_QUESTIONS,
  ...WORK_ENERGY_POWER_QUESTIONS,
  ...ROTATIONAL_MOTION_QUESTIONS,
  ...GRAVITATION_BOOK_QUESTIONS,
  ...ELASTICITY_BOOK_QUESTIONS,
  ...SURFACE_TENSION_BOOK_QUESTIONS,
  ...FLUID_MECHANICS_BOOK_QUESTIONS,
  ...THERMAL_PROPERTIES_BOOK_QUESTIONS,
  ...THERMODYNAMICS_PHYSICS_QUESTIONS,
  ...TRANSMISSION_OF_HEAT_QUESTIONS,
  ...KINETIC_THEORY_QUESTIONS,
  ...OSCILLATIONS_SHM_QUESTIONS,
  ...WAVES_ACOUSTICS_QUESTIONS,
  ...ELECTROSTATICS_BOOK_QUESTIONS,
  ...CAPACITANCE_QUESTIONS,
  ...CURRENT_ELECTRICITY_QUESTIONS,
  ...MAGNETISM_BOOK_QUESTIONS,
  ...MAGNETISM_MATTER_QUESTIONS,
  ...EMI_QUESTIONS,
  ...ALTERNATING_CURRENT_QUESTIONS,
  ...EM_WAVES_QUESTIONS,
  ...RAY_OPTICS_QUESTIONS,
  ...ALL_UNIVERSAL_RAY_OPTICS_QUESTIONS,
  ...UNIVERSAL_REFRACTION_PLANE_QUESTIONS,
  ...UNIVERSAL_TIR_QUESTIONS,
  ...WAVE_OPTICS_QUESTIONS,
  ...DUAL_NATURE_QUESTIONS,
  ...DUAL_NATURE_XRAYS_QUESTIONS,
  ...ATOMIC_NUCLEAR_PHYSICS_QUESTIONS,
  ...SELF_EVALUATION_TEST_26_QUESTIONS,
  ...SELF_EVALUATION_TEST_27_QUESTIONS,
  ...SELF_EVALUATION_TEST_28_QUESTIONS,
  ...SEMICONDUCTOR_AIIMS_AR_QUESTIONS,
  ...ELECTRONICS_CIRCUIT_AND_GRAPHICAL_QUESTIONS,
  ...ALL_COMMUNICATION_ADVANCED_QUESTIONS,
  ...ATOMS_PHYSICS_QUESTIONS,
  ...NUCLEI_PHYSICS_QUESTIONS,
  ...SEMICONDUCTORS_QUESTIONS
];
