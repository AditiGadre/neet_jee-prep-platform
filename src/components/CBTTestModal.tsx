    ...
    try {
      ...
      if (rawReqs) {
        ...
        if (Array.isArray(reqs)) {
          const testCode = (test.title || '').split(':')[0].trim().toUpperCase();
          const testId = (test.id || '').toUpperCase();
          const isApproved = reqs.some((r: any) => {
            ...
            return (
              ...
              rCode === testCode ||
              rCode === testId ||
              (r.rollNumber && r.rollNumber === rollNumber)
            );
          });
      ...
      console.warn('Error checking admin unlock status:', e);
    }
    return false;
  }, [isSundayTest, test.title, test.id, rollNumber]);
